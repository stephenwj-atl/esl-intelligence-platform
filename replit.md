...
12. **Live Data Ingestion System**: Located at `artifacts/api-server/src/services/data-ingestion/`. Four source adapters: Aqueduct (WRI water stress, 14 countries), IBTrACS (NOAA hurricane tracks CSV), Open-Data Jamaica (health facility CSVs from data.gov.jm), ArcGIS Jamaica (7 GOJ/MSET FeatureServer endpoints for flood, PRTR, mortality, protected areas, planning). Orchestrator runs pipelines sequentially, then triggers scoring engine. Scheduler uses pipeline name keys matching adapter names (hyphenated). Admin-only ingestion API routes at `/api/ingestion/run`, `/api/ingestion/run-all`, `/api/ingestion/status`. Scoring engine computes weighted country risk from ingested datasets and writes to `regional_indices`.
