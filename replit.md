...
**Database Schema Highlights:** Key tables include `projects` (expanded with `projectCategory`, `interventionType`, `capitalMode`, `latitude`, `longitude`, `hasSEA`, `hasESIA`, `persScore`, `interventionRiskScore`, `monitoringIntensity`, `lenderFramework`, `persBreakdown` JSONB, `interventionRiskProfile` JSONB), `portfolios`, `risk_history`, `covenants`, `esap_items`, `monitoring_events`, `audit_logs`, `pipelines`, `financial_impacts`, `outcomes`, `blended_structures`, `data_layers`, `project_data_layers`, `raw_data_cache`, `data_source_freshness` (with `ingestion_mode`: live/curated/hybrid), `regional_data`, and `ingestion_runs`. Schema alignment: `investment_amount` (real), `delay_risk_percent` (real), `cost_overrun_percent` (real), `covenant_breach_percent` (real), `decision_conditions` (jsonb) — Drizzle matches PostgreSQL. Demo seed script: `pnpm --filter @workspace/api-server run seed:demo` (7 projects across all capital modes and categories). PERS management scripts: `pnpm --filter @workspace/api-server run pers:backfill [--dry-run]` (scores unscored legacy projects), `pnpm --filter @workspace/api-server run pers:recalculate [--dry-run] [--country=Jamaica]` (recalculates all scored projects using latest regional_data). Country data lookup (`country-data-lookup.ts`) queries regional_data for governance (CPI), disaster history (EM-DAT), and INFORM Risk per country with provenance tracking (source, fallback status, timestamps).

**Authentication & Security:** Uses email/password with mandatory TOTP-based two-factor authentication (users cannot access any protected routes without completing 2FA setup). JWT session tokens stored in httpOnly cookies (no localStorage). bcrypt 12-round password hashing. AES-256-GCM encryption at rest for sensitive text fields (decisionOutcome, decisionInsight, reputationalRisk). decisionConditions is stored as plain jsonb (operational data, not sensitive PII). Security hardening: Helmet CSP, rate limiting (500 req/15min general, 20/15min auth), restricted CORS (auto-detects Replit domain), trust proxy for rate limiter, configurable session timeout via SESSION_TIMEOUT_HOURS env var (default 8h). Secrets stored in Replit Secrets (ENCRYPTION_KEY, ADMIN_BOOTSTRAP_SECRET, SESSION_SECRET) - never in .replit or source code.
