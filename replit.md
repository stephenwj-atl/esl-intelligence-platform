...
12. **Live Data Ingestion System**: Located at `artifacts/api-server/src/services/data-ingestion/`. Eighteen source adapters across 17 Caribbean countries and 101+ dataset types producing 4,844+ data records:
    ...
    - **NEPA EIA** (NEPA Jamaica multi-page web scraping — 156 regulatory documents: 40 EIA projects with annexes, 99 NRCA/TCPA board decisions (2019-2026, 10 pages), 2 enforcement listings, 7 public consultations, 34 archive docs; regulatory density scoring; uses Node.js `https` with `rejectUnauthorized: false` due to NEPA's incomplete SSL cert chain)
