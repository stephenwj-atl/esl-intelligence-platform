...
export default function NewProject() {
  ...
  const RangeSlider = ({ name, label, control }: { name: keyof FormData, label: string, control: any }) => {
    const value = watch(name) as number;
    return (
      <div className="space-y-3">
        <div className="flex justify-between items-center">
          <Label className="text-foreground/80">{label}</Label>
          <span className="text-xs font-mono font-bold px-2 py-1 bg-secondary rounded text-primary">{value}/10</span>
        </div>
        <Controller
          name={name}
          control={control}
  ...
  const Toggle = ({ name, label, description, control }: { name: keyof FormData, label: string, description: string, control: any }) => (
    <Controller
      name={name}
      control={control}
      render={({ field: { value, onChange } }) => (
        ...
        >
          <div className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center border transition-colors ${value ? 'bg-primary border-primary text-primary-foreground' : 'bg-transparent border-muted-foreground'}`}>
            {value && <ShieldAlert className="w-3 h-3" />}
          </div>
          <div>
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto pb-12">
        <div className="mb-8">
          <h1 className="text-3xl font-display font-bold text-foreground">New Environmental Assessment</h1>
          <p className="text-muted-foreground mt-2">Input project parameters to generate an intelligence report and risk score.</p>
        </div>
            ...
            <Card className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    {Object.values(CreateProjectInputProjectType).map(type => (
            ...
            <AnimatedContainer delay={0.3} className="space-y-8">
              ...
              <Card className="p-6 bg-primary/5 border-primary/20">
                ...
                <p className="text-sm text-muted-foreground mb-6">
                  The ESL Intelligence Engine will process these inputs against regional baselines to generate a unified risk profile and financial impact translation.
                </p>
                <Button type="submit" className="w-full py-6 text-base shadow-lg shadow-primary/20" disabled={isPending}>
                  {isPending ? (
                    <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Processing Intelligence...</>
                  ) : (
                    <><Save className="w-5 h-5 mr-2" /> Generate Intelligence Report</>
                  )}
                </Button>
              </Card>
            </AnimatedContainer>
